...
from Train.Model.NNMain import NNMain
import numpy as np
...
class TRNN(NNMain):
    def __init__(self, n_dv, n_disc, n_cont, n_label, dataset_train, dataset_test, verbose=1, lr=0.1,
                 deep_layers=[16, 16], batch_size=32, proj_layers=[5, 5, 5], proj_res={3: 1}, last_act=tf.nn.tanh,
                 scale_fac=[], activation=tf.nn.relu, random_seed=2019, l2_reg=0.0, is_reg=False):
        ...
        super(TRNN, self).__init__(n_dv=n_dv, n_disc=n_disc, n_cont=n_cont, batch_size=batch_size, activation=activation, learning_rate=lr, 
                                   l2_reg=l2_reg, verbose=verbose, random_seed=random_seed, 
                                   n_label=n_label, dataset_train=dataset_train,
                                   dataset_test=dataset_test, is_reg=is_reg)
    ...
    def _build_graph(self):
        ...
        deep_y = tf.reduce_mean(deep_y, axis=1)
