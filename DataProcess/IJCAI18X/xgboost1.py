...
from CONFIG import IJCAI18X_RAWDATA_PATH, IJCAI18X_DATASET_PATH, DOWNLOADS_PATH
...
# 划分训练集和测试集
def trainTestSplit(df, splitDate=pd.to_datetime('2018-09-23')):
    trainDf = df[df.context_timestamp<splitDate]
    testDf = df[df.context_timestamp>=splitDate]
...
if __name__ == '__main__':
    # 导入数据
    df = importDf('%s/IJCAI18/round1_ijcai_18_train_20180301.txt' % DOWNLOADS_PATH)

    # 特征处理
    df.drop_duplicates(inplace=True)
    originDf = pd.concat([df], ignore_index=True)
    originDf = dataCleaning(originDf)
    originDf = feaFactory(originDf)
    df = originDf[originDf.context_timestamp>='2018-09-19']
    trainDf, testDf = trainTestSplit(df, '2018-09-24')
    trainDf.to_csv("%s/train.csv" % IJCAI18X_RAWDATA_PATH, index=False)
    testDf.to_csv("%s/test.csv" % IJCAI18X_RAWDATA_PATH, index=False)
